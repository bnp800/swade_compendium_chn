...
class ExtractedEntry:
    ...
    link_display_texts: List[str] = field(default_factory=list)  # 链接显示文本列表
    # 保留 placeholders 用于向后兼容和内部使用
    ...
    def extract_entries(self, babele_data: Dict) -> List[ExtractedEntry]:
        ...
        for key, value in babele_entries.items():
            ...
            for field_name in self.translatable_fields:
                ...
                if is_html:
                    source_text = self.extract_text_from_html(stripped_content)
                else:
                ...
                # 同时用 extract_links 保留占位符映射（用于注入阶段）
